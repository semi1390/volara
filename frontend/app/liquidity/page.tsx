  ...
  return (
    <div>
      ...
      <div className="relative h-3 bg-card rounded-full overflow-hidden border border-white/10">
        ...
        <div
          className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full border-2 border-card shadow-lg transition-all duration-700"
...
function StatCard({ label, value, sub, color = 'text-white', delay = 0 }: {
  ...
  return (
    <div ref={ref} className={cn(
      ...
      <div className="text-text-secondary text-xs font-mono mb-2 md:mb-3">{label}</div>
      <div className={cn('font-mono font-bold text-xl md:text-2xl mb-1 transition-all group-hover:scale-105 origin-left', color)}>{value}</div>
      <div className="text-text-secondary text-xs font-mono">{sub}</div>
    </div>
  )
}
...
export default function LiquidityPage() {
  ...
  const realUtilization = poolBalanceMist > 0
    ? Math.min((totalExposure / poolBalanceMist) * 100, 100).toFixed(0) : '0'

  const hasRealHistory = totalPremiums > 0 && realPoolBalance > 0.1
  const realAPY = hasRealHistory
    ? Math.min(((totalPremiums / realPoolBalance) * 52 * 100), 200).toFixed(1) : null

  const walletBalance = balanceData
    ? parseFloat((parseInt(balanceData.totalBalance) / 1_000_000_000).toFixed(4)) : 0

  const userDepositValue = totalShares > 0 && poolBalanceMist > 0 && userShares > 0
    ? ((userShares / totalShares) * realPoolBalance).toFixed(4) : '0.0000'
  ...
  const handleDeposit = () => {
    ...
    signAndExecute({ transaction: tx as any }, {
      onSuccess: async () => {
        ...
        const newBalance = parseInt((newPool?.data?.data?.content as any)?.fields?.balance ?? '0') / 1_000_000_000
  ...
  const handleWithdraw = () => {
    ...
    tx.moveCall({
      ...
      arguments: [tx.object(process.env.NEXT_PUBLIC_LIQUIDITY_POOL_ID!), tx.pure.u64(sharesToWithdraw)],
    })
  ...
  const utilLevel = parseFloat(realUtilization) < 40 ? 'LOW' : parseFloat(realUtilization) < 70 ? 'MED' : 'HIGH'
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8">
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="font-syne font-extrabold text-4xl md:text-5xl text-white">Provide Liquidity</h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5 mb-8 md:mb-10">
          <StatCard label="Pool Balance" value={`${poolBalance} SUI`} sub="Total deposited" delay={0} />
          <StatCard
            label="Your Deposit"
            value={account ? `${userDepositValue} SUI` : '—'}
            sub={account ? (userShares > 0 ? 'Your share of pool' : 'No deposit yet') : 'Connect wallet'}
            color="text-profit"
            delay={100}
          />
          <StatCard
            label="Pool Utilization"
            value={`${realUtilization}%`}
            sub={utilLevel}
            color={parseFloat(realUtilization) < 40 ? 'text-profit' : parseFloat(realUtilization) < 70 ? 'text-yellow-400' : 'text-danger'}
            delay={200}
          />
          <StatCard
            label="APY (From Premiums)"
            value={realAPY ? `${realAPY}%` : '—'}
            sub={realAPY ? 'From real premiums' : 'No trading history yet'}
            color="text-profit"
            delay={300}
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 md:gap-8">
          <div className="space-y-6">
            ...
            <div className="card p-5 md:p-6 hover:border-primary/20 transition-all duration-300">
              {/* Tab toggle */}
              ...
              <div className="mb-4">
                <label className="text-text-secondary text-xs font-mono block mb-2">
                  Amount (SUI)
                  {account && <span className="ml-2 text-primary cursor-pointer hover:text-primary/80">Balance: {walletBalance} SUI</span>}
                </label>
                <div className="flex items-center gap-3 bg-background border border-white/10 rounded-xl px-4 py-3 focus-within:border-primary/40 transition-colors">
              ...
              {/* Estimated returns */}
              <div className="bg-background rounded-xl p-4 mb-4 border border-white/5">
              ...
              <div className="mb-5 md:mb-6">
                ...
                <RiskMeter level={utilLevel} />
              </div>

              {/* Action button */}
              <button
                onClick={activeTab === 'deposit' ? handleDeposit : handleWithdraw}
                className="w-full py-4 rounded-xl bg-gradient-to-r from-primary to-violet-500 text-white font-syne font-bold text-lg hover:shadow-glow-indigo hover:scale-[1.02] active:scale-[0.98] transition-all min-h-[52px]"
              >
                  ...
                  {(['7D', '30D', '90D', '1Y', 'ALL'] as const).map(f => (
                    <button key={f} onClick={() => setAnalyticsFilter(f)}
                      className={cn('px-2.5 md:px-3 py-1 rounded-lg text-xs font-mono transition-all flex-shrink-0',
                        analyticsFilter === f ? 'bg-primary/15 text-primary border border-primary/30 shadow-glow-indigo' : 'text-text-secondary hover:text-white'
                      )}>{f}</button>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-4 md:space-y-5">
            {/* AI insight */}
            <div className="ai-box p-4 md:p-5 hover:border-primary/30 hover:shadow-glow-indigo transition-all duration-300">
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                    ...
                    <div>
                      ...
                      <div className={cn('font-bold text-lg',
                        insight.risk_level === 'LOW' ? 'text-profit' : insight.risk_level === 'MED' ? 'text-yellow-400' : 'text-danger'
                      )}>{insight.risk_level}</div>
                    </div>
                  </div>
                  <div className="mt-4 pt-4 border-t border-white/10">
                    ...
                    <div className="mt-2 h-1.5 bg-background rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-primary to-profit rounded-full transition-all duration-1000" style={{ width: `${insight.confidence}%` }} />
                    </div>
                  </div>
                </>
              ) : (
            ...
            {/* Pool info */}
            <div className="card p-4 md:p-5 space-y-4 hover:border-primary/20 transition-all duration-300">
              ...
              {[
                ...
                <div key={item.label} className="flex items-center justify-between text-xs font-mono py-1 border-b border-white/5 last:border-0">
