...
function CountdownTimer({ targetDate }: { targetDate: Date }) {
  ...
  if (!mounted) return (
    <div className="text-center py-12">
      <div className="text-text-secondary font-mono text-sm mb-6">Next Settlement In</div>
      <div className="flex items-center justify-center gap-4">
        {['DAYS', 'HOURS', 'MINUTES', 'SECONDS'].map((label, i) => (
          <div key={label} className="flex items-center gap-4">
            <div className="text-center">
              <div className="font-mono font-bold text-8xl leading-none tabular-nums text-white">00</div>
              <div className="text-text-secondary font-mono text-xs mt-2 tracking-widest">{label}</div>
            </div>
            {i < 3 && <div className="text-6xl font-mono leading-none mb-4 text-white/30">:</div>}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="text-center py-12">
      <div className="text-text-secondary font-mono text-sm mb-6">Next Settlement In</div>
      <div className={cn('flex items-center justify-center gap-4', isUrgent && 'countdown-glow')}>
        {[
          ...
          <div key={unit.label} className="flex items-center gap-4">
            <div className="text-center">
              <div className={cn('font-mono font-bold text-8xl leading-none tabular-nums', isUrgent ? 'text-danger' : 'text-white')}>
                {unit.value}
              </div>
              <div className="text-text-secondary font-mono text-xs mt-2 tracking-widest">{unit.label}</div>
            </div>
            {i < 3 && (
              <div className={cn('text-6xl font-mono leading-none mb-4', isUrgent ? 'text-danger animate-pulse' : 'text-white/30')}>:</div>
            )}
          </div>
        ))}
      </div>
      <div className="mt-6 text-text-secondary font-mono text-sm">
  ...
  return (
    <div className="pt-24 pb-12 min-h-screen">
      <div className="max-w-[1440px] mx-auto px-4 md:px-8">
        ...
        {/* Countdown Hero */}
        <div className="card mb-10 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_360px] gap-6 md:gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Expiring Positions */}
            <div>
              <h2 className="font-syne font-bold text-2xl text-white mb-5">Positions Expiring This Week</h2>
              <div className="grid grid-cols-3 gap-4">
                {expectedPayouts.map(pos => {
                  const isITM = pos.calculatedPayout > 0
                  return (
                    <div key={pos.id} className={cn('card p-5 hover-lift', isITM ? 'hover:border-profit/30' : 'hover:border-white/10')}>
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-text-secondary text-xs font-mono">{pos.market}</span>
                      </div>
                      <div className="space-y-2 text-xs font-mono mb-4">
                        <div className="flex justify-between"><span className="text-text-secondary">Strike</span><span className="text-white">${pos.strike.toFixed(2)}</span></div>
                        <div className="flex justify-between"><span className="text-text-secondary">Current Price</span><span className="text-white">${settlementPrice.toFixed(2)}</span></div>
                        <div className="flex justify-between"><span className="text-text-secondary">Quantity</span><span className="text-white">{pos.qty}</span></div>
                      </div>
            ...
            {/* Settlement History */}
            <div>
              <h2 className="font-syne font-bold text-2xl text-white mb-5">Settlement History</h2>
                  ...
                  <tbody>
                    {SETTLEMENT_HISTORY.map((record, i) => (
                      <tr key={i} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                        <td className="px-4 py-3 font-mono text-xs text-text-secondary">{record.date}</td>
                        <td className="px-4 py-3 font-mono text-sm text-white">{record.market}</td>
                        <td className="px-4 py-3 font-mono text-sm text-white">${record.strike.toFixed(2)}</td>
                        ...
                        <td className={cn('px-4 py-3 font-mono text-sm font-bold', record.payout > 0 ? 'text-profit' : 'text-text-secondary')}>
                          {record.payout > 0 ? `+${record.payout.toFixed(2)}` : '0.00'} USDC
                        </td>
                        <td className="px-4 py-3">
                          <a href="#" className="flex items-center gap-1 font-mono text-xs text-primary hover:text-primary/70 transition-colors">
          ...
          <div className="space-y-5">
            ...
            <div className="card p-5">
              <h3 className="font-syne font-semibold text-white mb-5">Payout Calculator</h3>
              <div className="mb-4">
                <label className="text-text-secondary text-xs font-mono block mb-2">Expected Price at Expiry</label>
                <input type="range" min="0.50" max="1.00" step="0.01" value={settlementPrice}
              ...
              <div className="flex justify-between gap-1 mb-5">
                {[0.55, 0.65, 0.75, 0.90].map(p => (
                  <button key={p} onClick={() => setSettlementPrice(p)}
                    className="flex-1 py-1 text-xs font-mono rounded bg-white/5 text-text-secondary hover:bg-primary/15 hover:text-primary transition-all">
                    ${p.toFixed(2)}
                  </button>
                ))}
              </div>
              ...
              <div className="border-t border-white/10 pt-4">
                ...
                <div className={cn('font-mono font-bold text-3xl', totalCalculated > 0 ? 'text-profit' : 'text-text-secondary')}>
            ...
            {/* Info */}
            <div className="card p-5 space-y-3 text-xs font-mono">
