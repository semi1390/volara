...
function CountdownTimer({ targetDate }: { targetDate: Date }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setTime(getTimeUntilExpiry(targetDate)), 1000)
  ...
  return (
    <div className="text-center py-8 md:py-12">
      <div className="flex items-center justify-center gap-2 mb-4 md:mb-6">
        ...
        <div className="text-text-secondary font-mono text-sm">Next Settlement In</div>
      </div>
      <div className={cn('flex items-center justify-center gap-2 md:gap-6', isUrgent && 'countdown-glow')}>
        {units.map((unit, i) => (
          <div key={unit.label} className="flex items-center gap-2 md:gap-6">
            <div className="text-center group">
              <div className={cn(
                'font-mono font-bold text-5xl md:text-8xl leading-none tabular-nums transition-colors',
                isUrgent ? 'text-danger' : 'text-white group-hover:text-primary'
              )}>
            ...
            {i < 3 && (
              <div className={cn('text-3xl md:text-6xl font-mono leading-none mb-3 md:mb-4', isUrgent ? 'text-danger animate-pulse' : 'text-white/20')}>:</div>
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="pt-24 pb-12 min-h-screen">
      <div className="max-w-[1440px] mx-auto px-4 md:px-8">
        ...
        {/* Countdown Hero */}
        <div className={cn(
          'card mb-8 md:mb-10 relative overflow-hidden transition-all duration-700',
          'hover:border-primary/20 hover:shadow-glow-indigo',
          headerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        )}>
          <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-transparent" />
          <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary to-transparent" />
          {/* Keeper running badge */}
          <div className="absolute top-4 right-4 flex items-center gap-2 px-3 py-1.5 rounded-full bg-profit/10 border border-profit/20">
            <div className="w-1.5 h-1.5 rounded-full bg-profit status-pulse" />
            <span className="text-profit font-mono text-xs">Keeper Running 24/7</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_360px] gap-6 md:gap-8">
            ...
            <AnimatedCard delay={0}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {expectedPayouts.map((pos, i) => {
                  ...
                  return (
                    <div key={pos.id} style={{ transitionDelay: `${i * 100}ms` }} className={cn(
                      'card p-4 md:p-5 hover-lift transition-all duration-300',
                      isITM
                        ? 'hover:border-profit/30 hover:shadow-[0_0_20px_rgba(16,185,129,0.1)]'
                      ...
                      <div className="space-y-2 text-xs font-mono mb-3 md:mb-4">
                        {[
                          ...
                          ['Current Price', `$${settlementPrice.toFixed(2)}`],
                          ['Qty × Size', `${pos.qty} × ${CONTRACT_SIZE}`],
                        ].map(([k, v]) => (
                      ...
                      <div className={cn('p-3 rounded-xl border transition-all', isITM ? 'border-profit/20 bg-profit/10' : 'border-white/10 bg-white/3')}>
                      ...
                      {SETTLEMENT_HISTORY.map((record, i) => (
                        <tr key={i} className="border-b border-white/5 hover:bg-primary/5 transition-colors">
                          ...
                          <td className="px-3 md:px-4 py-3">
                            <span className={cn('text-xs font-mono font-bold px-2 py-1 rounded flex items-center gap-1 w-fit',
                              record.result === 'ITM' ? 'bg-profit/15 text-profit' : 'bg-danger/15 text-danger'
                            )}>
                              {record.result === 'ITM' ? <CheckCircle size={10} /> : <XCircle size={10} />}
                              {record.result}
                            </span>
                          </td>
                          <td className={cn('px-3 md:px-4 py-3 font-mono text-sm font-bold whitespace-nowrap',
                            record.payout > 0 ? 'text-profit' : 'text-text-secondary'
                          )}>
                            {record.payout > 0 ? `+${record.payout.toFixed(4)}` : '0.0000'} SUI
                          </td>
                          <td className="px-3 md:px-4 py-3">
                            <a href="#" className="flex items-center gap-1 font-mono text-xs text-primary hover:underline whitespace-nowrap">
          ...
          <div className="space-y-4 md:space-y-5">
            ...
            <AnimatedCard delay={50}>
              <div className="card p-4 md:p-5 hover:border-primary/20 transition-all duration-300">
                ...
                <div className="mb-4">
                  ...
                  <input type="range" min="0.50" max="1.00" step="0.01"
                    value={settlementPrice} onChange={e => setSettlementPrice(parseFloat(e.target.value))}
                    className="w-full accent-primary" />
                  <div className="flex justify-between text-xs font-mono text-text-secondary mt-1">
                    <span>$0.50</span>
                    <span className="text-primary font-bold text-sm">${settlementPrice.toFixed(2)}</span>
                    <span>$1.00</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between gap-1 mb-4 md:mb-5">
                  {[0.55, 0.65, 0.75, 0.90].map(p => (
                    <button key={p} onClick={() => setSettlementPrice(p)}
                      className={cn(
                        'flex-1 py-1.5 text-xs font-mono rounded transition-all min-h-[32px]',
                        settlementPrice === p ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-white/5 text-text-secondary hover:bg-primary/15 hover:text-primary'
                      )}>
                      ${p.toFixed(2)}
                    </button>
                  ))}
                ...
                <div className="space-y-2 mb-4 md:mb-5">
                  {expectedPayouts.map(pos => (
                    <div key={pos.id} className="flex items-center justify-between text-xs font-mono p-2.5 rounded-lg bg-background/50 border border-white/5 hover:border-primary/10 transition-colors">
                ...
                <div className="border-t border-white/10 pt-4">
                  ...
                  <div className={cn('font-mono font-bold text-2xl md:text-3xl transition-colors',
                    totalCalculated > 0 ? 'text-profit' : 'text-text-secondary'
                  )}>
            ...
            <AnimatedCard delay={100}>
              {account ? (
                ...
                <div className="space-y-3 card p-4">
                  <p className="text-center text-text-secondary font-mono text-sm">Connect wallet to track your settlements</p>
                  <ConnectButton />
                </div>
              )}
            </AnimatedCard>
            ...
            <AnimatedCard delay={150}>
              <div className="card p-4 md:p-5 space-y-3 hover:border-primary/20 transition-all duration-300">
                ...
                {[
                  ...
                  <div key={k} className="flex justify-between text-xs font-mono py-1 border-b border-white/5 last:border-0">
                    <span className="text-text-secondary">{k}</span>
                    <span className="text-white">{v}</span>
                  </div>
                ))}
